...
with open('research/proof_attempts/lemma91_gap_impossibility.md', 'r') as f:
    content = f.read()

new_content = """# Lemma 9.1b: The Impossibility of Fractal Fragmentation
...
For $p_k \ge 13$:
$$ I(M') > 2 - \\frac{56}{208} \\approx 1.73 $$

Since $M'$ is an odd number, to achieve $I(M') > 1.73$, it must have multiple prime factors. The smallest odd number with $I > 1.73$ is $M' = 45$ (with $I = 1.733$). Thus, $M' \\ge 45$.
Because $M'$ must be small enough to allow $p_k$ to cause a gap ($M' < p_k / I(M')$), this forces:
$$ p_k > 45 \\times 1.73 > 77 $$

So $p_k$ must be at least 79. If $p_k \\ge 79$, the bound tightens severely:
$$ I(M') > 2 - \\frac{320}{6478} \\approx 1.9506 $$

To achieve $I(M') > 1.95$, $M'$ must be much denser. The smallest odd number with $I > 1.95$ is $M' = 315$ (with $I = 1.98$).
This forces $p_k > 315 \\times 1.95 > 614$, so $p_k \\ge 617$.

If $p_k \\ge 617$, the bound becomes:
$$ I(M') > 2 - \\frac{2472}{382546} \\approx 1.9935 $$

To achieve $I(M') > 1.9935$, $M'$ must have at least 5 distinct prime factors. The smallest such odd number is $M' = 15015$.
This forces $p_k > 15015 \\times 1.9935 \\approx 29932$.
If $p_k \\ge 29932$, $I(M') > 1.9998$, forcing $M' > 10^8$.
This forces $p_k > 2 \\times 10^8$, which forces $I(M') > 1.9999999...$
...
with open('research/proof_attempts/lemma91_gap_impossibility.md', 'w') as f:
    f.write(new_content)

print("Proof rewritten.")
