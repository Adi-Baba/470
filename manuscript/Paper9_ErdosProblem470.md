...
By multiplying this established inequality by any existing divisor $d \in D(m_i)$, we obtain $d \cdot p_{i+1} \le d \cdot \sigma(m_i)$. Because every new divisor generated by the prime $p_{i+1}$ takes the form $d \cdot p_{i+1}$, this inequality formally guarantees that the smallest new scaled divisor is strictly less than or equal to the maximum possible subset sum of the prior unscaled divisors. This creates a rigorous combinatorial overlap, ensuring the new scaled divisors interleave seamlessly into the existing subset sums. Summing over the divisors (where $\Sigma_j$ represents the cumulative sum of all proper divisors up to $d_j$) algebraically establishes the general weak practical induction: $\Sigma_j \ge d_{j+1} - 1$ for all divisors beyond the initial computed fringe $K$. This formally establishes $M$ as weakly practical. Consequently, the proper divisors $D(M)$ form a gapless, contiguous block for all sums above the initial fringe $K$.
