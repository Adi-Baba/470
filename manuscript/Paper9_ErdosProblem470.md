...
**Proof.** 
We proceed by induction on the prime factors. For the base case $m_3 = 3 \cdot 5 \cdot 7 = 105$, the proper divisors are $\{1, 3, 5, 7, 15, 21, 35\}$, summing to 87. Direct evaluation shows the subset sums form a contiguous block $[15, 87]$, with fringe gaps at 2 and 14. Thus, $K = 15$ and $C_3 = 87$.

For any subsequent prime factor $p_{i+1}$, the new subset sums are $S_{i+1} = S_i + p_{i+1} S_i$. Since $1 \in S_i$, $S_{i+1}$ contains the shifted block $[K + p_{i+1}, C_i + p_{i+1}]$. For the bulk to remain contiguous, we require the overlap condition $p_{i+1} \le C_i - K + 1$.

Substituting $K = 15$ and $C_i = \sigma(m_i) - m_i$ (since the top fringe is 0 for these cores), the condition is $p_{i+1} \le \sigma(m_i) - m_i - 14$. 
Because $M$ resides in the dense regime ($k \ge 21$), the core $m_i$ contains many small prime factors, ensuring its capacity $\sigma(m_i) - m_i$ grows exponentially and vastly outpaces the next prime factor $p_{i+1}$. 

For example, at $i=3$, we require $p_4 \le 87 - 15 + 1 = 73$, which is trivially satisfied by the 4th odd prime ($p_4 = 11$). The bulk seamlessly merges into $[15, 98]$. By induction, the overlap condition is unconditionally satisfied at every step. The fringe $K$ never grows, and the core $M$ unconditionally inherits an unbroken contiguous bulk $[K, \sigma(M) - M]$. ∎
