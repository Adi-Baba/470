...
$$ L_{c \to 13/7^{-}} = \left(\frac{3}{2}\right) \left(\frac{7}{6}\right) \left(\frac{13}{12}\right) \left(\frac{29}{28}\right) \left(\frac{59}{58}\right) \cdots \approx 2.035 > 2 $$
...
For dense, square-free cores, relying solely on first-order cross-divisors is insufficient for large combinations. Instead, we evaluate the cumulative partial sum $\Sigma_i$ of *all* strictly smaller divisors. Since $M$ contains small distinct primes to maximize abundancy, the dense combinatorics of all preceding sub-factors unconditionally satisfies $\Sigma_i \ge d_{i+1}$.

If $M$ deviates from this ideal square-free structure by incorporating large prime powers (e.g., $11^2$), the abundancy index strictly decreases compared to using distinct smaller primes. To compensate for this drop in abundancy and maintain $I(M) > 1.8$, the core size $M$ must expand exponentially to include more prime factors. This massive expansion of $M$ proportionally widens the required Minkowski interval length $L = \frac{\sigma(M)-M}{q}$. Therefore, any localized gaps introduced by prime powers are strictly contained within the larger capacity interval $L$, allowing the subset-sum projection to span across any localized subset-sum gaps. ∎
