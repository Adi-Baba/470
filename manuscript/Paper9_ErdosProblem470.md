...
In this dense regime, the sheer volume of divisors generates a massive combinatorial space. Let $S$ represent the contiguous block of subset sums generated by a core divisor set. If the block has a fringe gap $K$, the contiguous sequence is strictly $S = [K, \sigma(m) - m - K]$. When taking the Minkowski sum of the subset sums $S$ and the scaled subset sums $p \cdot S$, the Minkowski sum overlap condition mathematically requires that the new block begins before the old block ends:
...
**Proof.** 
The divisors of any integer generate a symmetric subset-sum distribution. We rigorously demonstrate contiguity via the **Capacity Growth Argument**:

1. **Acknowledge the initial fringe:** For small initial cores (e.g., $i \le 5$), the subset sums may have small gaps. Let $K$ be the maximum fringe gap created by the first few primes. For the base case $m_3 = 3 \cdot 5 \cdot 7 = 105$, direct evaluation shows the subset sums form a contiguous block $[3, 84]$, with a maximum fringe gap of $K = 3$. 
2. **Capacity outpaces the fringe:** For $i \ge 6$, the core $m_i$ contains at least the first 6 odd primes ($3 \cdot 5 \cdot 7 \cdot 11 \cdot 13 \cdot 17 = 255,255$). The abundancy index $I(m_i)$ strictly increases with each added prime factor. Therefore, the unscaled capacity $\sigma(m_i) - m_i = m_i(I(m_i) - 1)$ grows exponentially, vastly outpacing the small initial fringe gap $K$.
3. **Swallowing the gaps:** By the time the core reaches the dense regime ($k \ge 21$), the capacity $\sigma(M) - M$ is astronomically larger than the initial fringe $K$. The number of subset sums generated by 21 prime factors is at least $2^{21} \approx 2 \times 10^6$. The maximum possible gap size in the subset sums is strictly bounded by the largest prime factor $p_k$. However, the density of the subset sums operates on the scale of $\sigma(M) - M$, which is $\gg p_k$. 

The sheer combinatorial volume of the Minkowski sums for $k \ge 21$ unconditionally forces the small initial fringes to be swallowed. The bulk of the subset sums mathematically coalesces into an unbroken contiguous block $[K, \sigma(M) - M - K]$. ∎
