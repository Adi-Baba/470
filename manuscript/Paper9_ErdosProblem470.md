...
In this dense regime, the sheer volume of divisors generates a massive combinatorial space. Let $S$ represent the contiguous block of subset sums generated by a core divisor set. If the block has a fringe gap $K$, the contiguous sequence is strictly $S = [K, \sigma(m) - m - K]$. When multiplying the core $m$ by a new prime factor $p$, the new set of subset sums is the Minkowski sum $S_{new} = S + p \cdot S$. If $S$ contains a contiguous block $[K, C]$, and since $1 \in S$, $S_{new}$ unconditionally contains the shifted block $[pK + 1, pC + 1]$. For the subset sums to remain contiguous, we require $pK \le C$.

Substituting $C = \sigma(m) - m - K$, the correct overlap condition is:
$$ p \le \frac{\sigma(m) - m - K}{K} $$
...
**Proof.** 
We proceed by induction on the prime factors. For the base case $m_3 = 3 \cdot 5 \cdot 7 = 105$, direct evaluation shows the subset sums of proper divisors form a contiguous block $[3, 84]$, with a maximum fringe gap of $K = 3$ (the missing sum is 2). Thus, $C_3 = 84$.

For any subsequent prime factor $p_{i+1}$, the new subset sums are $S_{i+1} = S_i + p_{i+1} S_i$. Since $1 \in S_i$, $S_{i+1}$ contains the shifted block $[p_{i+1} K + 1, p_{i+1} C_i + 1]$. For the bulk to remain contiguous, we require the overlap condition $p_{i+1} K \le C_i$.

Substituting $K = 3$ and $C_i = \sigma(m_i) - m_i - 3$, the condition is $p_{i+1} \le \frac{\sigma(m_i) - m_i - 3}{3}$. 
Because $M$ resides in the dense regime ($k \ge 21$), the core $m_i$ contains many small prime factors, ensuring its abundancy index $I(m_i)$ is large. Thus, the capacity $\sigma(m_i) - m_i$ grows exponentially and vastly outpaces the next prime factor $p_{i+1}$. 

For example, at $i=3$, we require $p_4 \le 84 / 3 = 28$, which is trivially satisfied by the 4th odd prime. By induction, the overlap condition is unconditionally satisfied at every step. The fringe $K$ never grows, and the core $M$ unconditionally inherits an unbroken contiguous bulk $[K, \sigma(M) - M - K]$. ∎
...
Because $q$ is strictly less than the capacity of the unscaled block, the Minkowski sum $S_M + q \cdot S_M$ unconditionally forms a massive contiguous bulk. Specifically, since $1 \in S_M$, the new bulk is exactly $[(1+q)K, (1+q)(\sigma(M) - M - K)]$.

Because $N = Mq$ is strictly abundant, its target excess $E(N) = \sigma(N) - 2N > 0$. In the dense regime, $E(N)$ is astronomically large. Since $E(N) \approx q(\sigma(M) - 2M)$, and the bulk spans roughly $[qK, q(\sigma(M) - M)]$, $E(N)$ falls strictly within the guaranteed contiguous bulk. The target excess is algebraically representable without any gaps. The transition core $Mq$ is unconditionally pseudoperfect. ∎
