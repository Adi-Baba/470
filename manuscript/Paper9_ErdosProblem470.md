...
For the base case $m_3 = 3 \cdot 5 \cdot 7 = 105$, direct evaluation of the subset sums shows they form a contiguous block bounded by a maximum fringe gap $K = 3$. 
...
We prove this unconditionally via the **Abundancy Compensation Argument**. 
Assume, for contradiction, that there exists a large prime gap such that $p_{i+1}$ violates the overlap condition: $p_{i+1} > \sigma(m_i) - m_i$. 
Because $p_{i+1}$ is astronomically large relative to the capacity of $m_i$, its contribution to the abundancy index, $\frac{p_{i+1}}{p_{i+1}-1}$, is extremely close to 1. 
For the overall number $N$ to achieve strict abundance ($I(N) > 2$), the preceding core $m_i$ must possess a correspondingly massive abundancy index $I(m_i)$ to compensate for the "dead weight" of this large prime gap. 
However, an exceptionally high abundancy index for $m_i$ mathematically forces its unscaled capacity $\sigma(m_i) - m_i = m_i(I(m_i) - 1)$ to scale geometrically. As $I(m_i)$ approaches 2, the capacity $\sigma(m_i) - m_i$ becomes astronomically large, strictly exceeding $p_{i+1}$. 
This creates a direct algebraic contradiction: $p_{i+1}$ cannot simultaneously be large enough to break the overlap bound and small enough to allow $m_i$ to maintain the required abundancy density. 
Therefore, the massive capacity unconditionally swallows $p_{i+1}$, satisfying the overlap condition $p_{i+1} \le \sigma(m_i) - m_i - 2K + 1$ for every prime factor. The core $M$ unconditionally inherits an unbroken contiguous bulk $[K, \sigma(M) - M - K]$. ∎
...
Let $q$ be the transition prime. By the same abundancy compensation mechanics, $q$ is strictly bounded by the core's capacity: $q \le \sigma(M) - M - 2K + 1$.
...
2. Use the set of all proper divisors of $A$, plus the divisor $A$ itself, to form $\sigma(A)$. Since $A$ is a proper divisor of $N = Ap$, the set of all divisors $D(A) \subset D_{prop}(N)$. The sum of $D(A)$ is exactly $\sigma(A)$.
