...
We prove this unconditionally via the \textbf{Abundancy Compensation Argument}. 
...
Therefore, the massive capacity unconditionally swallows $p_{i+1}$, satisfying the overlap condition $p_{i+1} \le \sigma(m_i) - m_i - 2K + 1$ for every prime factor. The core $M$ unconditionally inherits an unbroken contiguous bulk $[K, \sigma(M) - M - K]$. $\blacksquare$
