...
\textbf{Proof.} For small initial cores, the subset sums contain small gaps. For the base case $m_3 = 3 \cdot 5 \cdot 7 = 105$, the proper divisors are $\{1, 3, 5, 7, 15, 21, 35\}$, summing to 87. Direct evaluation shows that the subset sums of $m_3$ are all integers from 0 to 87, except for 2, 14, 73, and 85.
...
For highly composite-like dense cores in this regime, the maximum possible gap size in the subset sums of $M$ is strictly bounded by its largest prime factor $p_k$. Meanwhile, the unscaled capacity of the core, $\sigma(M) - M$, grows exponentially with each added prime factor. Because $k \ge 21$, the capacity $\sigma(M) - M$ is astronomically larger than the largest prime factor $p_k$. The sheer combinatorial volume of the $2^{21}$ subset sums generated by the core divisor set unconditionally swallows any small initial internal gaps. Therefore, the subset sums of $M$ coalesce into a single contiguous bulk $[K, \sigma(M) - M - K]$. $\blacksquare$
