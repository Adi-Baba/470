...
To prove that the transition core $Mq$ is pseudoperfect, we project the target excess across the $q$-scaled divisors. We seek to represent $E(N)$ as $q \cdot y + x$, where both $y$ and $x$ are subset sums of the unscaled divisors $D(M)$. By Theorem 6.1 (proven below), the subset sums of $D(M)$ form a contiguous block $[0, \sigma(M)-M]$ above a small computable fringe. 
...
While Theorem 6.1 strictly guarantees $q < \sigma(M)$, for dense odd cores $M$ in the $k \ge 21$ regime, the unscaled capacity $\sigma(M)-M$ is massively larger than the transition prime $q$. Algebraically, the interval length evaluates to $L \gg 1$. 
