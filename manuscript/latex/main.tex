...
\textbf{Proof.} We proceed by induction on the prime factors. For the base case $m_3 = 3 \cdot 5 \cdot 7 = 105$, direct evaluation shows the subset sums of proper divisors form a contiguous block $[3, 84]$, with a maximum fringe gap of $K = 3$ (the missing sum is 2). Thus, $C_3 = 84$.
...
For example, at $i=3$, we require $p_4 \le 84 / 3 = 28$, which is trivially satisfied by the 4th odd prime. By induction, the overlap condition is unconditionally satisfied at every step. The fringe $K$ never grows, and the core $M$ unconditionally inherits an unbroken contiguous bulk $[K, \sigma(M) - M - K]$. $\blacksquare$
